...
|---|---|---|---|---|---|
| customer-web | 游戏商城、游戏详情、订单/权益、咨询、工单进度和评价体验 | 浏览器短期 UI 状态；不拥有业务真相 | 浏览器导航 | platform-api 的 HTTPS/WebSocket | agent-service、数据库、中间件 |
| staff-web | 客服工作台、管理员工单、退款/账号处置、角色模块配置、知识库和看板 | 浏览器短期 UI 状态；不拥有业务真相 | 浏览器导航 | platform-api 的 HTTPS/WebSocket | agent-service、数据库、中间件 |
